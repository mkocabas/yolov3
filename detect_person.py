...
def detect(
    ...
    for i, (path, img, im0, vid_cap) in enumerate(dataloader):
        ...
        if det is not None and len(det) > 0:
            ...
            for si, pred in enumerate(det):
                if pred[-1] == 0:
                    scale = float(pred[3] - pred[1]) / 200.
                    center = [float(pred[0] + pred[2])/2., float(pred[1] + pred[3])/2.]
                    jdict.append({
                        'image': os.path.abspath(path),
                        'scale': scale,
                        'center': center,
                        'width': float(pred[2] - pred[0]),
                        'height': float(pred[3] - pred[1])
                    })
    ...
    with open(str(Path(output) / 'bbox_results.json'), 'w') as file:
